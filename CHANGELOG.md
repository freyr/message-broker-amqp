...
### Changed
- **BREAKING:** `freyr/message-broker-contracts` dependency upgraded from `^0.1` to `^0.3`
- **BREAKING:** `freyr/identity` upgraded from `^0.4` to `^1.0` (UUID v7 to ULID) — moved to require-dev
- Enable `failOnDeprecation` in PHPUnit configuration to match CI strictness
